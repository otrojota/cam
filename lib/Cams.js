...
class Cams extends ZModule {
    ...
    getCameraEvents(cameraCode, fromTime, toTime) {
        return new Promise((resolve, reject) => {
            let timeZone = config.getConfig().timeZone;
            let t0 = moment.tz(fromTime, timeZone);
            let t1 = moment.tz(toTime, timeZone);
            let camera = this.getCameras().find(cam => cam.code == cameraCode);
            camera.getEventsResume(t0, t1, timeZone)
                .then(camResume => resolve(camResume))
